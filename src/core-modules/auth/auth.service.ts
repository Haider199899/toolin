...
export class AuthService {
  ...
  async verifyToken(idToken: string): Promise<IIdTokenResponse> {
    try {
      ...
      throw new UnauthorizedException('Unauthorized user. Invalid access token.');
    }
  }
  ...
  async ssoGoogleAndFacebook(idTokenDTO: IdTokenDTO, authMethod: string) {
    const userData: IIdTokenResponse = await this.verifyToken(idTokenDTO.idToken);
  ...
  async updatePassword(user : IIdTokenResponse,updatePassword : UpdatePasswordDTO) {
